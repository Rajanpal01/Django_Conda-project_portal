...
def documentation_download_link(request):
    para = request.session.get('Form_value_name')
    return render(request, 'documentation_download_link.html',{'para':para})
...
def on_click_here_project(request,id):
    ...
    return render(request, 'on_click_here_project.html',{'obb':obb, 'para':para,})
...
def PPT(request):
    para = request.session.get('Form_value_name')
    return render(request, 'PPT_Location.html',{'para':para})
